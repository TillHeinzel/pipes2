...
namespace pipes
{
  ...
  {
    finish_impl(source, connect_links(ops, finalSink));
  }
} // namespace pipes
...
namespace pipes
{
  ...
  auto append(RawNodes<Ops...> earlierOps, PSink<Ts...> sink)
    PIPES_FWD(PSink{sink.finalSink, earlierOps + sink.ops});

  template<class... T1s, class... T2s>
  auto append(Source<T1s...> source, PSink<T2s...> sink)
